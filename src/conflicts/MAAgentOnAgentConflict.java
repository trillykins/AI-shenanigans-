...
public class MAAgentOnAgentConflict {
	...
	public static void moveAgentOnAgentNoBox(Agent agentToMove, Agent agentToStay, Box agentToMoveBox) {
		if (agentToStay.getStepInPlan() == 0)
			...
			/*
			 * the agentToMove is locked in a narrow corridor by boxes and the
			 * agentToStay
			 */
			...
			if (!putAgentInACorner) {
				if (newPlanAgentToMove != null && !newPlanAgentToMove.isEmpty()) {
					...
					/*
					 * in pacman it was nessesary for to add a no op for the
					 * agentToStay, else our detection would detect conflicts in
					 * a loop
					 */
					Node noOp = null;
					...
					if (agentToMove.getStepInPlan() == 0) {
						/*
						 * need to create a node where the agent is at current
						 * position
						 */
					...
					/*
					 * we need to copy the first node, but remember to remove it
					 * since it still has the not-noOp command
					 */
	...
	public static void moveAgentOnAgentWithBox(Agent agentToMove, Agent agentToStay, Box boxToMove) {
		if (agentToStay.getStepInPlan() == 0)
			...
			/*
			 * if the new plan was null it means that it wasnt possible to find
			 * a route away from agentToStay.
			 */
	...
	public static boolean putAgentInACorner(Agent agentToStay, Agent agentToMove, LinkedList<Node> newPlanAgentToMove, Box boxToMove) {
		/*
		 * we want to put him in a corner (make use of free fields calculation)
		 * until agent
		 */
		/*
		 * at this point we know it was not possible to replan either of the
		 * agents, therefore we need to put agentToStay in a corner, and then
		 * replan agent to move
		 */
