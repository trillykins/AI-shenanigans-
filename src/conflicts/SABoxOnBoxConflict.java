...
//		agent.initialState.walls.add(new Position(agent.getPosition()));
		agent.initialState.agentRow = agent.getPosition().getX();
		agent.initialState.agentCol = agent.getPosition().getY();
		System.err.println(agent.initialState.boxes.values());
...
//		Strategy strategy = new StrategyBFS();
		...
		LinkedList<Node> newPlanAgentToMove = s.search(new StrategyBFS(), agent.initialState, SearchType.MOVE_OWN_BOX);

		agent.setPlan(newPlanAgentToMove);
