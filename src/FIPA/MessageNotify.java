...
public class MessageNotify implements Runnable {//
	...
	public void run() {
		...
		if(message != null) {
			...
			System.err.println("Agent color " + receiver.getCol() + "has received the message");
			System.err.println("Request agent color " + receiver.getCol() + " to " + message.getContent());
			
			//Test code
			//Should decide the logical of the replanning (How to update the current solution)
			newAction = new Node(null);
			int col = message.getReceiver().getCurrentCol();
			int row = message.getReceiver().getCurrentRow();
			...
			newAction.boxes = receiver.getInitialState().boxes;
			newAction.goals = receiver.getInitialState().goals;
